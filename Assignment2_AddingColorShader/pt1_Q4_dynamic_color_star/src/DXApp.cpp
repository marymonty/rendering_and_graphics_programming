...
DXApp::~DXApp()
{
	...
	ReleaseAndDeleteCOMobject(mpcbColor);
...
void DXApp::InitDemo()
{
	...
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColorPlus.hlsl", "VS", "vs_4_0", &pVSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	// MARY
	// the layout needs to change to account for the int ColorNum
	// now the second element of layout is labeled with semantic "COLORNUM" and is of DXGI_FORMAT
	// of an 8 bit unsigned int
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "COLORNUM", 0, DXGI_FORMAT_R8_UINT, 0, 16 , D3D11_INPUT_PER_VERTEX_DATA, 0 }
	};
	...
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColorPlus.hlsl", "PS", "ps_4_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	// create the star pattern of vertices
	// this is now of type SimpleVertex, which is a struct that 
	// contains a vertex Position and Color
	SimpleVertex vertices[] =
	{
		// Star Pattern contains six separate triangles
		// north pointing, south pointing, east pointing, and west pointing triangles
		// two central triangles to make the yellow square

		// north pointing triangle
		{ Vect(0.0f, 1.0f, 0), 1 },	// Top 
		{ Vect(0.25f, 0.25f, 0), 1 },	// Bottom right 
		{ Vect(-0.25f, 0.25f, 0), 1 },	// Bottom left 
		...
		// center square is at the same verts as north trianges bottom points
		// and south triangles bottom points, but needs redeclaration to be diff color
		{ Vect(0.25f, 0.25f, 0), 0 },	// top right 
		{ Vect(-0.25f, 0.25f, 0), 0 },	// top left 
		{ Vect(-0.25f, -0.25f, 0), 0 },	// Bottom left 
		{ Vect(0.25f, -0.25f, 0), 0 }	// Bottom right
	};
	...
	// MARY
	// ByteWidth = sizeof(type of vertex) * num of verts
	// updating number of verts to be 16 
	...
	// MARY
	// listing the vertices in counter clockwise order
	WORD indices[] =
	{
		...
		3, 4, 5,		// south pointing
		6, 7, 8,		// east pointing
		9, 11, 10,		// west pointing
		12, 13, 14,		// upper center triangle
		12, 14, 15		// lower center tirangle
	};

	bd.Usage = D3D11_USAGE_DEFAULT;

	// MARY
	// ByteWidth = sizeof(type of indice) * number of verts per triange * number of trianges
	// we have 4 triangles so its sizeof(WORD) * 3 * 6
	bd.ByteWidth = sizeof(WORD) * 3 * 6;        
	...
	// MARY
	// we need to add in the Constant Buffer 
	// we are adding onto the D3D11_BUFFER_DESC bd 
	// we need to define this new usage
	bd.Usage = D3D11_USAGE_DEFAULT;
	// define the bite width as the size of the color struct
	bd.ByteWidth = sizeof(CBColor);
	// set the bind flag to bind to the constant buffer
	bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	// set the CPUAccessFlags to be 0 as to not need access
	bd.CPUAccessFlags = 0;

	// try to connect the device to the constant buffer
	// CreateBuffer takes ( const pointer to a D3D11_BUFFER_DESC structure that describes the buffer, 
	//	const pointer to a D3D11_SUBRESOURCE_DATA structure that describes the initialization data; use NULL to allocate space only, 
	//	Address of a pointer to the ID3D11Buffer interface for the buffer object created ) 
	hr = md3dDevice->CreateBuffer(&bd, nullptr, &mpcbColor);
	// assert this buffer creation succeeded by checking the return
	assert(SUCCEEDED(hr));

	// set the immediate context constant buffer
	// I am using PSSetConstantBuffers and not VSSetConstantBuffers because I am changing the color
	// in the Pixel shader and not in the Vertex shader
	// PSSetConstantBuffers takes ( uint index into the device's zero-based array to begin setting constant buffers to,
	//	uint number of buffers to set, a const pointer to the array of constant buffers being given to the device )
	md3dImmediateContext->PSSetConstantBuffers(0, 1, &mpcbColor);

	// how to set the color dynamically through the constant buffer
	CBColor cbMyColor;
	cbMyColor.col[0] = Colors::Yellow;
	cbMyColor.col[1] = Colors::Red;
	// update the contexts subresource constant buffer
	// UpdateSubresource takes ( pointer to destination resource, zero-based index that identifies the destination subresource,
	//	A pointer to a box that defines the portion of the destination subresource to copy the resource data into (null means no offset),
	//	A pointer to the source data in memory, uint size of one row of the source data, uint size of one depth slice of source data )
	md3dImmediateContext->UpdateSubresource(mpcbColor, 0, nullptr, &cbMyColor, 0, 0);
...
{	
	...
	// MARY
	// DrawIndexed first param is the index count, found with indices per triangle * number of triangles
	// so in our case, 3 * 6
	md3dImmediateContext->DrawIndexed(3 * 6, 0, 0);  
