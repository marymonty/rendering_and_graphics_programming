...
void DXApp::InitDemo()
{
	...
	// MARY
	// We need to switch the input layout from { {Pos}, {Color} } to { {Color}, {Pos} }
	// to match the new struct of the vertex buffer, we also need to update the 5th param
	// which is the byte offset within the struct now that Color has no byte offset (0) 
	// and Position has an offset (16 bytes due to the size of the Color Vect)
	...
	// MARY
	// changing verts from { Pos, Color } to { Color, Pos }
	// Just running code as is with this change, the project creates an empty viewport
	// we also need to change the input layout to make this work
	SimpleVertex vertices[] =
	{
		// change to { Color, Pos }
		{ Colors::Red, Vect(0.0f, 0.5f, 0) },
		{ Colors::Lime, Vect(0.5f, -0.5f, 0) },
		{ Colors::Blue, Vect(-0.5f, -0.5f, 0) }
	};

	D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd, sizeof(bd));
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex) * 3;	// size of verts * num of verts
	...
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(WORD) * 3 * 1;        // 3 vertices needed for each triangles in a triangle list
...
{	
	...
	md3dImmediateContext->DrawIndexed(3 * 1, 0, 0);  // 3 indices for each triangle in a list
