...
void DXApp::InitDirect3D()
{
	...
	//*
	D3D11_TEXTURE2D_DESC descDepthTextureBuf;
	descDepthTextureBuf.Width = mClientWidth;
	descDepthTextureBuf.Height = mClientHeight;
	descDepthTextureBuf.MipLevels = 1;
	descDepthTextureBuf.ArraySize = 1;
	descDepthTextureBuf.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	descDepthTextureBuf.SampleDesc = sampDesc;
	descDepthTextureBuf.Usage = D3D11_USAGE_DEFAULT;
	descDepthTextureBuf.BindFlags = D3D11_BIND_DEPTH_STENCIL;
	descDepthTextureBuf.CPUAccessFlags = 0;
	descDepthTextureBuf.MiscFlags = 0;

	ID3D11Texture2D* pDepthStencil;
	hr = md3dDevice->CreateTexture2D(&descDepthTextureBuf, NULL, &pDepthStencil);
	assert(SUCCEEDED(hr));

	// Create the depth stencil view
	D3D11_DEPTH_STENCIL_VIEW_DESC descDSV;
	ZeroMemory(&descDSV, sizeof(descDSV));
	descDSV.Format = descDepthTextureBuf.Format;
...
void DXApp::InitDemo()
{
	...
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColor3D.hlsl", "VS", "vs_4_0", &pVSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColor3D.hlsl", "PS", "ps_4_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	{
		{ Vect(-1.0f, -1.0f, -1.0f), Colors::Black },
		{ Vect(-1.0f, +1.0f, -1.0f), Colors::Lime },
		{ Vect(+1.0f, +1.0f, -1.0f), Colors::Yellow },
		{ Vect(+1.0f, -1.0f, -1.0f), Colors::Red },
		{ Vect(-1.0f, -1.0f, +1.0f), Colors::Blue },
		{ Vect(-1.0f, +1.0f, +1.0f), Colors::Cyan },
		{ Vect(+1.0f, +1.0f, +1.0f), Colors::White },
		{ Vect(+1.0f, -1.0f, +1.0f), Colors::Magenta }
	};

	D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd, sizeof(bd));
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex) * 8;	// size of verts * num of verts
	...
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(WORD) * 3 * 12;        // 3 vertices needed for each triangles in a triangle list
	...
	// MARY
	// Create the two separate constant buffers
	...
	// MARY
	// Connect the buffer to the render pipeline
	// we have to specify the register numbers of the constant buffers!
	...
	mWorld1 = Matrix(IDENTITY);		// Returns a 4x4 identity matrix
...
void DXApp::DrawScene()
{
	...
	// We need to update the values of the matrices on the GPU
	
	// MARY
	// rather than sending one object then the other, 
	// we can just send the camera data once, stored in the ViewProjData const buffer
	// Sending the data for the camera matrices
	ViewProjData vpdata;
	...
	// Sending data for object #1
	WorldColorData wcdata;
	// the world matrices were initialized in the InitDemo function
	wcdata.mWorld = mWorld1;
	wcdata.Color = Colors::Aquamarine;
	md3dImmediateContext->UpdateSubresource(mpcbWorldColorBuffer, 0, nullptr, &wcdata, 0, 0);

	// render Object #1
	md3dImmediateContext->DrawIndexed(3 * 12, 0, 0);  // 3 indices for each triangle in a list

	// Sending the data for Object #2
	wcdata.mWorld = mWorld2;
	wcdata.Color = Colors::Crimson;
	md3dImmediateContext->UpdateSubresource(mpcbWorldColorBuffer, 0, nullptr, &wcdata, 0, 0);
...
DXApp::~DXApp()
{
	...
	// MARY
	// release the updated constant buffer handlers
