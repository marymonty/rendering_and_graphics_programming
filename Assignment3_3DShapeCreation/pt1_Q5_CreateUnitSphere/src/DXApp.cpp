...
void DXApp::InitDemo()
{
	...
	// constructing Models
	unitBoxModel = new Model(md3dDevice, Model::PreMadeModels::UnitBox);
	...
	// create the constant buffer needed for SimpleColor3D shader
	bd.Usage = D3D11_USAGE_DEFAULT;
	...
	// Initialize the worldmat, one for each rendered object so there's no overlap
...
void DXApp::DrawScene()
{
	...
	/********** Connect the shader to the pipeline *************/
	...
	unitBoxModel->SetToContext(md3dImmediateContext);
	// Render Object #1
	unitBoxModel->Render(md3dImmediateContext);
	...
	/********** Connect the second shader to the pipeline *************/
	...
	// now our constant buffer is WVPmatrices
	WVPmatrices wvpdata;
	...
	// we only need to update the world matrix and color in our second pyramid
...
DXApp::~DXApp()
{
	...
	// deleted allocated Model memory
	delete unitBoxModel;
