...
void DXApp::InitDemo()
{
	...
	// MARY
	// need to switch from using SimpleColor3D to using ColorSelected3D shader file
	...
	StandardVertex* vertices = new StandardVertex[8];
	vertices[0].set(Vect(-0.5f, -0.5f, -0.5f), Colors::Black);
	vertices[1].set(Vect(-0.5f, +0.5f, -0.5f), Colors::Lime);
	vertices[2].set(Vect(+0.5f, +0.5f, -0.5f), Colors::Yellow);
	vertices[3].set(Vect(+0.5f, -0.5f, -0.5f), Colors::Red);
	vertices[4].set(Vect(-0.5f, -0.5f, +0.5f), Colors::Blue);
	vertices[5].set(Vect(-0.5f, +0.5f, +0.5f), Colors::Cyan);
	vertices[6].set(Vect(+0.5f, +0.5f, +0.5f), Colors::White);
	vertices[7].set(Vect(+0.5f, -0.5f, +0.5f), Colors::Magenta);

	D3D11_BUFFER_DESC bd;
	//ZeroMemory(&bd, sizeof(bd));
	//bd.Usage = D3D11_USAGE_DEFAULT;
	//bd.ByteWidth = sizeof(StandardVertex) * 8;	// size of verts * num of verts
	...
	TriangleByIndex* indices = new TriangleByIndex[12];
	indices[0].set(0, 1, 2);
	indices[1].set(0, 2, 3);	// back face		

	indices[2].set(4, 6, 5);
	indices[3].set(4, 7, 6);	// front face

	indices[4].set(4, 5, 1);
	indices[5].set(4, 1, 0);	// right face

	indices[6].set(3, 2, 6);
	indices[7].set(3, 6, 7);	// left face

	indices[8].set(1, 5, 6);
	indices[9].set(1, 6, 2);	// top face

	indices[10].set(4, 0, 3);
	indices[11].set(4, 3, 7);	// bottom face

	//bd.Usage = D3D11_USAGE_DEFAULT;
	//bd.ByteWidth = sizeof(TriangleByIndex) * 12;        // 1 TriangleByIndex for each triangles in a triangle list
	...
	//md3dImmediateContext->IASetIndexBuffer(mpIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
	//md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);


	// MARY
	// constructing three Models using the Model constructor
	// Model(ID3D11Device* dev, PreMadeModels pm);
	unitCubeModel = new Model(md3dDevice, Model::PreMadeModels::UnitBox);
	// pModel2 is not needed since it is a replica of unitCubeModel 
	// so we only need to create the UnitBox object once even though we render it twice
	unitPyramidModel = new Model(md3dDevice, Model::PreMadeModels::UnitPyramid);

	// data has been copied in Model
	delete[] vertices;
	delete[] indices;

	// Have the model set its vert & index buffers to context
	//unitCubeModel->SetToContext(md3dImmediateContext);
	// need to set the new model vert & index buffers to context
	...
	// MARY
	// create the two separate constant buffers needed
	// for the ColorSelected3D shader file
...
void DXApp::DrawScene()
{
	...
	// MARY
	// only send the camera view and projection matrices once
	ViewProjData vpdata;
	...
	// set individual rendered object's world matrix and color then update the subresource
	WorldColorData wcdata;
	wcdata.mWorld = mWorld;
	wcdata.Color = Colors::Beige;
	md3dImmediateContext->UpdateSubresource(mpcbWorldColorBuffer, 0, nullptr, &wcdata, 0, 0);
	// make sure the correct model is calling SetToContext
	...
	// for object 2 (repeated cube) we only need to update
	// the world matrix and color, then we can render Model1 again
	wcdata.mWorld = mWorld2;
	wcdata.Color = Colors::Firebrick;
	md3dImmediateContext->UpdateSubresource(mpcbWorldColorBuffer, 0, nullptr, &wcdata, 0, 0);
	// Render Object #2
	unitCubeModel->Render(md3dImmediateContext);

	//render unitPyramid
	wcdata.mWorld = mWorld3;
