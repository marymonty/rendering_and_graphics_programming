...
void DXApp::DrawScene()
{
	...
	// MARY
	// doing the necessary multiplication to get the WVP matrix
	// to send with the ColorSelected3DOptimized constant buffer
	// for Q1 this is not optimized
	Matrix WVP = mWorld * mCam.getViewMatrix() * mCam.getProjMatrix();
	pShColor->SendWVPColor(WVP, Colors::Yellow );
