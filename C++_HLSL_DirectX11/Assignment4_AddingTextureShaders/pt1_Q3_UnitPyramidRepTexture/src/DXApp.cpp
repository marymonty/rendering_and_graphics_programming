...
void DXApp::InitDemo()
{
	...
	// MARY
	// loading the correct texture file
	hr = LoadFromTGAFile(L"../Assets/Textures/AlignmentTest.tga", nullptr, myTexture);
	assert(SUCCEEDED(hr));
	...
	//pModel1 = new Model(md3dDevice, Model::PreMadeModels::UnitBox);
	//pModel1 = new Model(md3dDevice, Model::PreMadeModels::UnitBoxRepeatedTexture);
	//pModel1->SetToContext(md3dImmediateContext);
	
	// MARY
	// we are creating the UnitPyramidRepTexture model now and setting it to context
	// since it is the only thing we are rendering
	pyramidTextured = new Model(md3dDevice, Model::PreMadeModels::UnitPyramidRepeatedTexture);
	pyramidTextured->SetToContext(md3dImmediateContext);
...
void DXApp::DrawScene()
{
	...
	// --- Render Object #1;
	// MARY
	// make sure we are rendering our pyramid texture object
	pyramidTextured->Render(md3dImmediateContext);
