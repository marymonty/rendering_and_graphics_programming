// MARY
// TextureMultiLight
// copy of my new ColorLightTexture hlsl, but with added
// functionality of light arrays
...
cbuffer LightParameters : register (b1)
{
	...
	// MARY
	// need to update point and spot lights to be arrays
...
struct VS_OUTPUT
{
	...
	// need to add our texture coordinates (UV)
	float2 Tex : TEXCOORD;
	float4 Norm : NORMAL;
};
...
float4 PS( VS_OUTPUT input ) : SV_Target
{
	...
	// MARY
	// now we need to loop through our Point lights array
	int index = 0;
	while (index < 3)
	{
		// send the point light at the current index
	...
	while (index < 3)
	{
		// send the spot light at the current index
