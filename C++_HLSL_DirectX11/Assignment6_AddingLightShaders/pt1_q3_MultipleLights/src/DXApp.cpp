...
void DXApp::InitDemo()
{
	...
	// ShaderColorLightTexture shader
	pShaderColLightTex = new ShaderColorLightTexture(md3dDevice);

	// MARY
	// new ShaderTextureMultiLight
	pShaderTexMultiLight = new ShaderTextureMultiLight(md3dDevice);
	...
	// MARY
	// all we need for this demo is the terrain
	pTexPlane = new Texture(md3dDevice, L"../Assets/Textures/sandgrass.tga", D3D11_FILTER_MIN_MAG_MIP_LINEAR);
	pTerrain = new TerrainModel(md3dDevice, L"../Assets/Textures/MaryHM.tga", 300, 30, 0, 30, 30);
	mWorld4 = Matrix(SCALE, 1, 1, 1) * Matrix(TRANS, -3, -4, 0);
	// and positions for the spot lights
	pos0 = Vect(0, 0, -4);
	pos1 = Vect(-8, 0, -4);
	pos2 = Vect(8, 0, -4);
...
void DXApp::DrawScene()
{
	...
	// MARY
	// set the new pShaderTexMultiLight to context and set directional 
	// and multiple point and spot light params
	pShaderTexMultiLight->SetToContext(md3dImmediateContext);
	pShaderTexMultiLight->SetDirectionalLightParameters(Vect(-1, -1, 1).getNorm(),
		.1 * Vect(1, 1, 1), .5 * Vect(1, 1, 1), Vect(1, 1, 1));

	//int index, Vect& pos, float r, Vect& att, Vect& dir, float spotExp, Vect& amb, Vect& dif, Vect& sp
	pShaderTexMultiLight->SetSpotLightParameters( 0, pos0, 2000, .5 * Vect(1, 0, 0), 
		-pos0.getNorm(), 50, .2 * Vect(1, 1, 1), 3*Vect(1, 1, 1), 1*Vect(1, 1, 1, 150 ));
	pShaderTexMultiLight->SetSpotLightParameters(1, pos1, 1000, .5 * Vect(1, 0, 0),
		-pos1.getNorm(), 25, .2 * Vect(1, 1, 1), 3 * Vect(1, 1, 1), 1 * Vect(1, 1, 1, 150));

	//int index, const Vect& pos, float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp
	pShaderTexMultiLight->SetPointLightParameters(0, Vect(0, 10, 0), 10, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));
	pShaderTexMultiLight->SetPointLightParameters(1, Vect(30, 10, -10), 20, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	// send the cam matrices and eyepos to shader and set the light parameters
	pShaderTexMultiLight->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());
	Vect eyepos;
	mCam.getPos(eyepos);
	pShaderTexMultiLight->SendLightParameters(eyepos);
	


	pShaderTexMultiLight->SendWorldAndMaterial(mWorld4, Colors::White, Colors::White, Vect(1, 1, 1, 100));
	pShaderTexMultiLight->SetTextureResourceAndSampler(pTexPlane);
