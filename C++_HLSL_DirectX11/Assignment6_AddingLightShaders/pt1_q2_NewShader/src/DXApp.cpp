...
void DXApp::InitDemo()
{
	...
	// MARY
	// use our new ShaderColorLightTexture shader
	pShaderColLightTex = new ShaderColorLightTexture(md3dDevice);
	// we set the point, directional, and spot lights in Draw so we can switch shaders
	// also set the shader to context in the Draw stage
	...
	// MARY
	// pre-loading my models and textures
	uPyr = new Model(md3dDevice, Model::PreMadeModels::UnitPyramidRepeatedTexture);
	...
	pTexPlane = new Texture(md3dDevice, L"../Assets/Textures/sandgrass.tga", D3D11_FILTER_MIN_MAG_MIP_LINEAR);
	pTerrain = new TerrainModel(md3dDevice, L"../Assets/Textures/MaryHM.tga", 30, 8, 0, 20, 20);
...
void DXApp::DrawScene()
{
	...
	// MARY
	// set the new pShaderColLightTex to context and set spot / directional / point light params
	pShaderColLightTex->SetToContext(md3dImmediateContext);
	pShaderColLightTex->SetSpotLightParameters( pos2, 4000, .5 * Vect(1, 0, 0), 
		-pos2.getNorm(), 50, .2 * Vect(1, 1, 1), 3*Vect(1, 1, 1), 1*Vect(1, 1, 1, 150 ));
	pShaderColLightTex->SetDirectionalLightParameters(Vect(-1, -1, 1).getNorm(),
		.1 * Vect(1, 1, 1), .5 * Vect(1, 1, 1), Vect(1, 1, 1));
	pShaderColLightTex->SetPointLightParameters(Vect(0, 10, 0), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	// send the cam matrices and eyepos to shader
	pShaderColLightTex->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());
	Vect eyepos;
	mCam.getPos(eyepos);
	pShaderColLightTex->SendLightParameters(eyepos);
	
	// render our shapes one on top of another to see them all
	// add textures to the shader before rendering
	// we don't need to set textures to context, that is done through the Texture class
	// which is called through our SetTextureResourceAndSampler function
	pShaderColLightTex->SendWorldAndMaterial(mWorld1 * Matrix(TRANS, 0, 2*cosf( mTimer.TotalTime()), 0), Colors::White, Colors::White, Vect(1,1,1, 100));
	pShaderColLightTex->SetTextureResourceAndSampler(pAlignmentTex);
	uPyr->SetToContext(md3dImmediateContext);
	uPyr->Render(md3dImmediateContext);

	pShaderColLightTex->SendWorldAndMaterial(mWorld3 * Matrix(TRANS, 0, 2 * cosf(mTimer.TotalTime()), 0), Colors::White, Colors::White, Vect(1, 1, 1, 100));
	pShaderColLightTex->SetTextureResourceAndSampler(pCrateTex);
	uBox->SetToContext(md3dImmediateContext);
	uBox->Render(md3dImmediateContext);

	pShaderColLightTex->SendWorldAndMaterial(mWorld4, Colors::White, Colors::White, Vect(1, 1, 1, 100));
	pShaderColLightTex->SetTextureResourceAndSampler(pTexPlane);
	// the TerrainModel class sets to context within the Render function
	pTerrain->Render(md3dImmediateContext);


	///*
	// to render out the flat planes and circling box we switch back to the texture-less
	// pShaderColLight. set the shader to context, send the light params, set the eyepos
	// then render our planes and box
	...
	pShaderColLight->SendWorldAndMaterial(mWorld2, Colors::Black, Colors::Black, Vect(1, 1, 1, 100));
