...
// MARY
// creating the NUMVERTS variable
static const int NUMVERTS = 60;
//assert(NUMVERTS >= 3);
...
void DXApp::InitDirect3D()
{
	...
	// MARY 
	// make sure FillMode is in FILL_WIREFRAME mode to see the wireframe
...
void DXApp::InitDemo()
{
	...
	// MARY
	// there are NUMVERTS amount of vertices
	// the verts need to be spaced out based on the number of verts
	// and the location in a circle
	
	Vect vertices[NUMVERTS];
	// the 0th vertex will always be at (1, 0, 0)
	vertices[0] = Vect(1.0f, 0.0f, 0);
	// we have the radian increment value (2 pi / number of verts)
	float radIncrementVal = (6.28318531 / NUMVERTS);
	// we have the current radian value (where we are in the circle)
	float radVal = 0;
	int i = 1;
	while (i < NUMVERTS)
	{
		// move to the next evenly spaced radian measurement in the circle
		radVal += radIncrementVal;
		// the vertice location is found by using the current radian value (radVal)
		// and finding the cos of that radVal for the x part of the vertex, and the
		// sin of that radVal for the y part of the vertex
		vertices[i] = Vect(cosf(radVal), sinf(radVal), 0);
		i++;
	}
	...
	// in the vertice array, which in our case is the const NUMVERTS
	bd.ByteWidth = sizeof(Vect) * NUMVERTS;	
	...
	// MARY
	// remember to list the verts counter clockwise!

	// we can find the number of indices with : (NUMVERTS - 2) * 3
	//	which we got from detrmining that the first 3 verts make one triangle, and each additional
	//	vert adds another triangle, so we know 4 verts = 6 indicies (two triangles since 3 verts makes 1,
	//	4 verts makes two), 5 verts = 9 indicies (three triangles), this math can be found simply by taking
	//	(NUMVERTS - 2) * 3
	const int numIndices = (NUMVERTS - 2) * 3;
	WORD indices[numIndices];
	// currentVert keeps track of what vertice we are on to create the current triangle
	WORD currentVert = 1;
	i = 0;
	while (i < numIndices)
	{
		// we go three indices at a time to make the traingles
		// the first triangle point will always be 0 (vert (1, 0, 0))
		indices[i] = 0;
		indices[i + 1] = currentVert;
		indices[i + 2] = currentVert + 1;
		i += 3;
		// we need to increment the current vert so we can move on to making the next triangle
		currentVert++;
	}
	...
	// OR just sizeof(WORD) * number of indices numIndices, which is (NUMVERTS - 2)*3 
...
{	
	...
	// the number of indices per triangle, with the total number of triangles
	// we can find the number of indices with : (NUMVERTS - 2) * 3
	md3dImmediateContext->DrawIndexed((NUMVERTS - 2) * 3, 0, 0); 
