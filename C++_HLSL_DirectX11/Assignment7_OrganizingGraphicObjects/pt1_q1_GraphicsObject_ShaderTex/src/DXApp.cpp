...
void DXApp::InitDemo()
{
	...
	// MARY
	// initialize our shaders and textures first in order to initialize out GraphicObject texture objects
	pShaderTex = new ShaderTexture(md3dDevice);
	pTex1 = new Texture(md3dDevice, L"../Assets/Textures/CrateFace.tga");
	pTex2 = new Texture(md3dDevice, L"../Assets/Textures/Dice.tga");
	GraphObj3 = new GraphicObject_Texture(pShaderTex, pModel1);
	GraphObj3->SetTexture(pTex1);
	GraphObj4 = new GraphicObject_Texture(pShaderTex, pModel2);
	GraphObj4->SetTexture(pTex2);
	// GraphObj5 is the same as 3, with the crate texture initially set
	GraphObj5 = new GraphicObject_Texture(pShaderTex, pModel1);
	GraphObj5->SetTexture(pTex1);
	// make an extra texture to test in draw function
	pTex3 = new Texture(md3dDevice, L"../Assets/Textures/CubeFaces.tga");


	// Initialize the world matrix
	mWorld1 = Matrix(IDENTITY);
	mWorld2 = Matrix(SCALE, .5f, .5f, .5f) * Matrix(TRANS, 2, 1, 3);
	mWorld3 = Matrix(SCALE, .25f, .25f, .25f) * Matrix(TRANS, 1, 1, 0);

	// MARY
	// set our new graphic object worlds
	GraphObj3->SetWorld(mWorld1);
	GraphObj4->SetWorld(mWorld2);
	// set GraphObj5's world to mWorld1 for now as well
...
void DXApp::DrawScene()
{
	...
	// MARY
	// change to our texture shaders then render our textured graphic objects
	pShaderTex->SetToContext(md3dImmediateContext);
	pShaderTex->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());

	// --- Render Object #1;
	GraphObj3->Render();
	// move object 5 and change the texture so we can test that these functions work
	GraphObj5->SetWorld(mWorld3);
	GraphObj5->SetTexture(pTex3);
	GraphObj5->Render();
