...
void DXApp::InitDemo()
{
	...
	pModel1 = new Model(md3dDevice, Model::UnitPyramidRepeatedTexture);
	pModel2 = new Model(md3dDevice, Model::UnitBoxSixFaceTexture);

	// MARY
	// initialize our shaders and GraphicObjects
	pShaderTexLight = new ShaderColorLightTexture(md3dDevice);
	GraphObj3 = new GraphicObject_TexLight(pShaderTexLight, pModel1);
	GraphObj4 = new GraphicObject_TexLight(pShaderTexLight, pModel2);
	pTex1 = new Texture(md3dDevice, L"../Assets/Textures/AlignmentTest.tga");
	...
	// MARY
	// set our graphic objects materials
	GraphObj3->SetMaterial(Colors::HotPink, Colors::HotPink, Vect(1, 1, 1, 100));
	GraphObj4->SetMaterial(Colors::Honeydew, Colors::Honeydew, Vect(1, 1, 1, 100));

	// MARY
	// set our graphic object textures
...
void DXApp::DrawScene()
{
	...
	pShaderTexLight->SetPointLightParameters(Vect(0, 10, 0), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));
	pShaderTexLight->SetSpotLightParameters(Vect(1, 0, 6), 4000, .5 * Vect(1, 0, 0),
		-Vect(1, 0, 6).getNorm(), 50, .2 * Vect(1, 1, 1), 3 * Vect(1, 1, 1), 1 * Vect(1, 1, 1, 150));
	mCam.getPos(eyepos);
	pShaderTexLight->SendLightParameters(eyepos);
	GraphObj4->Render();
