...
void DXApp::InitDemo()
{
	...
	// MARY
	// constructing a new Model using the Model constructor
	// Model(ID3D11Device* dev, PreMadeModels pm);
	// in order to specify using the UnitPyramid
	// Construct the model from verts and indices
	//pModel1 = new Model(md3dDevice, vertices, 8, indices, 12);
	pModel1 = new Model(md3dDevice, Model::PreMadeModels::UnitPyramid);
...
void DXApp::DrawScene()
{
	...
	WVPmatrices cb;
	cb.mWorld = mWorld;
	cb.mView = mCam.getViewMatrix();
	cb.mProjection = mCam.getProjMatrix();
	md3dImmediateContext->UpdateSubresource(mpcbWVPBuffer, 0, nullptr, &cb, 0, 0);
