...
void DXApp::InitDemo()
{
	...
	/****************************************/
	//MARY
	// demo 1 with HMtest16
	// demo 2 with sandgrass
	//mTexPlane = new Texture(md3dDevice, L"../Assets/Textures/HMtest16.tga", D3D11_FILTER_MIN_MAG_MIP_LINEAR);
	mTexPlane = new Texture(md3dDevice, L"../Assets/Textures/sandgrass.tga", D3D11_FILTER_MIN_MAG_MIP_LINEAR);
	// MARY
	// HMtest is 256x256, HMtest16 is 16x16, HMtest4 is 4x4
	//pTerrain = new TerrainModel(md3dDevice, L"../Assets/Textures/HMtest16.tga", 50, 10, 0, 3, 2);
	//ID3D11Device* dev, LPCWSTR heightmapFile, float len, float maxheight, float ytrans, int RepeatU, int RepeatV
	pTerrain = new TerrainModel(md3dDevice, L"../Assets/Textures/MaryHM.tga", 1000, 100, -5, 30, 30);

	pModel1 = new Model(md3dDevice, Model::PreMadeModels::UnitBoxRepeatedTexture);
    mTex1 = new Texture(md3dDevice, L"../Assets/Textures/CrateFace.tga", D3D11_FILTER_MIN_MAG_MIP_LINEAR);

	// Initialize the worldmat for model1
	...
	//mCam.setOrientAndPosition(Vect(0, 1, 0), Vect(0, 0, 30), Vect(0, 12, -5));
...
void DXApp::DrawScene()
{
	...
	// --- Render plane;
	// MARY
	// implement the Render call
	pTerrain->Render(md3dImmediateContext);  // Homework...

	/*	Commenting out the box render
