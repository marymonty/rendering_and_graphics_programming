...
void DXApp::InitDemo()
{
	...
	hr = LoadFromTGAFile(L"../Assets/Textures/GOOD_skybox.tga", nullptr, myTexture);
	...
	// MARY
	// choosing to Clamp filter, so if we go out of bounds of the UV texture
	// we are just clamping the sky to the rest of the box
	...
	//boxTextured = new Model(md3dDevice, Model::PreMadeModels::SkyBoxTexture);
	//boxTextured->SetToContext(md3dImmediateContext);

	// MARY
	// make a skybox object and decide the scaled size, then set to context
	skyBoxTextured = new SkyBox(md3dDevice, 300);
	// we set to context in skyBox Render call
...
void DXApp::DrawScene()
{
	...
	// MARY 
	// render our skybox
	skyBoxTextured->Render(md3dImmediateContext);
